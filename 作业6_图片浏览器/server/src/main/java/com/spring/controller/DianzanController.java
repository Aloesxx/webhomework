...
public class DianzanController extends BaseController {
     ...
     * 用户列表
     */
    @RequestMapping("/dianzan_list_username")
    ...
    public String insert() {
        ...
        Map res = Query.make("dianzan").where("biao", Request.get("biao")).where("biaoid", Request.get("biaoid")).where("username", session.getAttribute("username")).find();
        if (res != null && !res.isEmpty()) {
            Query.make("dianzan").where("id", res.get("id")).delete();
            return showSuccess("已取消点赞");
        }
    ...
    public String batch() {
        ...
        if (
            ...
            if (ids != null) Query.make("dianzan").where("id", "in", ids).delete(); // 执行删除
        }
